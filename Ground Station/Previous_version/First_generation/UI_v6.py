...
class GroundStationUI(QWidget):
    def __init__(self):
        ...
        #tab.setLayout(left_panel)
        #tab.setLayout(right_panel)

    def create_status_tab(self, status: str) -> QWidget:
    ...
    def initUI(self):
        ...
        for status in ["IDLE", "READY", "ASCENDING", "DESCENDING", "RETRIEVAL"]:
            self.tabs.addTab(self.create_status_tab(status), status)
