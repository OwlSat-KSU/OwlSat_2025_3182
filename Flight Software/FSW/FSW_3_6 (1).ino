...
float altitude = bmp.readAltitude() - reference_altitude;
return (String(altitude - reference_altitude) + "," + String(temp) + "," + String(pressure));
}
...
String getTime(){
  int C_hh , C_mm , C_ss;
  C_hh = hh_c - hour.toInt();
  C_mm = mm_c - minute.toInt();
  C_ss = ss_c - second.toInt();
  ...
  return String(hour.toInt() + C_hh)+":" + String(minute.toInt() + C_mm)+":" + String(second.toInt() + C_ss);
}
...
if(token[0] == "3182"){
  ...
  if(token[1] == "CX"){
    ...
    delay(2000);
    boomStick(88);
    Xbee_send("MOTOR FINISHED, TEST ENDED");
