...
void boomStick(int frequency){
// 96 - 86 == no rotation
// 86 - 35 == reverse rotation
// 96 - 145 == forward rotation
motor.attach(frequency);
...
void Handle_Commands(String sentence){
  ...
  // token[0] = CMD
  // token[1] = Team ID
  // token[2] = Command type, ex. CX, ST
  // token[3] = Value/Device
  // token[4] = ON/OFF -- only for MEC command
...
if(token[1] == "3182"){
  ...
  if(token[2] == "CX"){
    ...
    if(token[3] == "BoomStick"){}
    else if(token[3] == "PinRelease"){}
    else if(token[3] == "Beep"){}
  }
    ...
    if(token[3] == "ENABLE"){m = 1;}
    else if(token[3] == "ACTIVATE"){ SIM_ACTIVE = true;}
    else if(token[3] == "DISABLE"){m = 0;
    SIM_ACTIVE = true;}
  }


  else if(token[2] == "SIMP"){SIMP = token[4];}
  else if(token[2] == "CAL"){}
  else if(token[2] == "TEST"){
    packet = "";
    if(BMP280() != ""){
      packet += "|BMP280 OK|";
