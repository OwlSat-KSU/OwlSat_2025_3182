...
if(m){
  if(SIM_ACTIVE){
  pressure = SIMP.toInt();
  altitude = 44330*pow((1 - (pressure/101.325)),1/5.256);}
...
if(Lakshay == 90){
  ...
  if(min_angle > angle)
    min_angle = angle;
  if(angle <= max_angle && angle >= 0.6)
  Lakshay = map(Lakshay,0.6,max_angle,96,120);
  if(angle <= 0.4 && angle >= min_angle)
  Lakshay = map(Lakshay,min_angle,0.4,120,140);
  motor.write(Lakshay);
  }
...
delay(500);
packet = "";
packet = BMP280()+"," + getVoltage()+"," + "0" +"," + Mag()+"," + "0" +"," + "0";
Xbee_send(packet);
